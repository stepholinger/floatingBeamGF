function mcmc_object = mseed_mcmc_setup(param_func)
...
sigma, num_iterations, L_type, data_path, fs, event_bounds] = param_func();
                              
% make model object
model = loadParameters(L,f_max,t_max,h_i,h_w);

% read in data from mseed file
data = read_data_mcmc(data_path,fs,f_max,event_bounds);
